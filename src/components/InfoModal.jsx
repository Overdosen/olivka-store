...
export default function InfoModal({ isOpen, onClose, title, type, src }) {
      ...
      return (
        <div key={i} className="flex items-start gap-4 mb-4 last:mb-0">
          ...
          <div className="flex-1 min-w-0 text-[#524f25]/90">
            {urlMatch ? (
              <p>
                {currentLine.split(urlMatch[0])[0]}
                <a 
                  href={urlMatch[0]} 
                  target="_blank" 
                ...
                {currentLine.split(urlMatch[0])[1]}
              </p>
            ) : isEmail ? (
              <a 
  ...
  // Вирішуємо проблему з картинками: звужуємо вікно для каруселі, щоб вертикальні фото заповнили його
      ...
      >
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto bg-white/40 custom-scrollbar-minimal">
            ...
            {/* Content for Text files or Static text */}
            {(type === 'text_file' || type === 'static_text') && (
              <div 
                className="max-w-4xl mx-auto w-full"
                style={{ paddingLeft: '40px', paddingRight: '40px' }}
              >
                {/* АДЕКВАТНИЙ ВЕРХНІЙ ВІДСТУП */}
                <div style={{ height: '50px' }} />
                
                <div className="text-[#524f25]/90 font-light text-base md:text-xl leading-relaxed space-y-4">
                  {loading ? (
