...
const PAYMENT_OPTIONS = [
  ...
  {
    id: 'iban',
    label: '100% оплата за реквізитами IBAN',
    icon: <Banknote size={24} style={{ color: '#c4a882' }} />,
    desc: ''
  }
];
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (payment !== 'liqpay') {
        ...
        // 2. Send Email Confirmation (Always for non-liqpay)
        console.log('[Checkout] Triggering email confirmation...');
        await fetch('/api/orders/send-confirmation', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ orderId: newOrderId })
        }).then(res => {
          if (!res.ok) console.error('[Checkout] Email API returned error status:', res.status);
          else console.log('[Checkout] Email confirmation triggered successfully');
      ...
      // Перенаправлення на сторінку успіху для всіх користувачів (щоб бачили реквізити для COD / IBAN)
      const successMethod = payment === 'iban' ? 'iban' : 'cod';
      router.push(`/payment/success?order_id=${newOrderId}&orderNo=${newOrder?.order_number || ''}&method=${successMethod}`);
    } catch (err) {
      toast.dismiss(loadingToast);
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                    {PAYMENT_OPTIONS.map(opt => (
                      <div key={opt.id}>
                        ...
                        {/* Inline Payment Info for IBAN */}
                        {payment === 'iban' && opt.id === 'iban' && (
                          <motion.div
                            initial={{ opacity: 0, y: -10 }}
                            animate={{ opacity: 1, y: 0 }}
                            style={{ overflow: 'hidden' }}
                          >
                            <div style={{ 
                              ...
                              border: '1px solid rgba(82,79,37,0.08)',
                                ...
                                <p style={{ fontSize: '0.85rem', color: '#524f25', lineHeight: '1.5', margin: 0 }}>
                                  Реквізити IBAN зʼявляться одразу після натискання «Підтвердити замовлення». Ми також продублюємо їх вам на e-mail.
                                </p>
                            </div>
                          </motion.div>
                        )}
