...
import { Package, Search, Tags, Edit2, X, Plus } from 'lucide-react';
  ...
  return (
    <div className="space-y-5 pb-10">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
        <PageHeader 
          title="Категорії" 
          subtitle="Огляд категорій товарів та налаштування."
        />
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
        <div className="relative w-full md:max-w-md">
          <input
            ...
            className="w-full px-4 py-3 bg-white rounded-lg border border-stone-200 focus:outline-none focus:ring-2 focus:ring-stone-400/20 focus:border-stone-400 transition-all text-sm"
          />
        </div>
      </div>
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-stone-100 bg-stone-50/50">
                <th className="px-6 py-5 text-[10px] uppercase tracking-wider font-semibold text-stone-500">Категорія</th>
                <th className="px-6 py-5 text-[10px] uppercase tracking-wider font-semibold text-stone-500 text-center hidden sm:table-cell">Порядок</th>
                <th className="px-6 py-5 text-[10px] uppercase tracking-wider font-semibold text-stone-500 text-right">Залишок (од)</th>
                <th className="px-6 py-5 text-[10px] uppercase tracking-wider font-semibold text-stone-500 text-right">Дії</th>
              </tr>
            </thead>
              ...
              {loading ? (
                [...Array(3)].map((_, i) => (
                  <tr key={i} className="animate-pulse">
                    <td className="px-6 py-5"><div className="h-6 bg-stone-100 rounded w-32" /></td>
                    <td className="px-6 py-5 hidden sm:table-cell"><div className="h-4 bg-stone-100 rounded w-10 mx-auto" /></td>
                    <td className="px-6 py-5"><div className="h-5 bg-stone-100 rounded w-12 ml-auto" /></td>
                    <td className="px-6 py-5"><div className="h-8 bg-stone-100 rounded w-8 ml-auto" /></td>
                  </tr>
                ))
                ...
                <tr>
                  <td colSpan="4">
                    <EmptyState
                      ...
                      description={searchTerm ? 'Спробуйте змінити пошуковий запит' : 'Додайте свою першу категорію'}
                    />
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={cat.id} className="hover:bg-stone-50/60 transition-colors group">
                    <td className="px-6 py-5">
                      <div className="flex items-center space-x-3">
                        <div className="w-8 h-8 rounded-lg bg-stone-100 flex items-center justify-center flex-shrink-0 border border-stone-200/50">
                          <Package className="w-4 h-4 text-stone-400" />
                        </div>
                        <div className="flex flex-col min-w-0">
                          <span className="font-medium text-stone-800 text-sm truncate">{cat.name}</span>
                          <span className="text-[10px] text-stone-400 uppercase tracking-tighter sm:hidden truncate">ID: {cat.id}</span>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-5 text-center text-stone-500 font-mono text-xs hidden sm:table-cell">{cat.sort_order}</td>
                    <td className="px-6 py-5 text-right font-semibold text-stone-800 tabular-nums">
                      {cat.total_stock || 0} шт
                    </td>
                    <td className="px-6 py-5 text-right">
                      <button 
                        onClick={() => setEditingCategory(cat)}
                        className="p-1.5 text-stone-400 hover:text-stone-700 hover:bg-stone-100 rounded-lg transition-all inline-flex"
                        title="Редагувати"
                      >
                        <Edit2 size={16} />
                      </button>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <input 
                  ...
                  className="w-full px-4 py-3 bg-stone-50 border border-stone-200 rounded-md focus:bg-white focus:border-stone-400 outline-none transition-all text-sm font-medium"
                />
              </div>
              <div className="space-y-1.5">
            ...
            <div className="space-y-1.5">
              ...
              <textarea 
                ...
                className="w-full px-4 py-3 bg-stone-50 border border-stone-200 rounded-md focus:bg-white focus:border-stone-400 outline-none transition-all text-sm font-medium resize-none"
